...
void can_transmit_frame(const twai_message_t *message, const char *label) {
    ...
    /* Locally generated sensor messages are rule-engine sources.  Never let
     * the engine feed its own base+5 command output back into a source. */
...
void canReceiveDispatch(void *arg)
{
    ...
    while (true) {
        ...
        while (drained < 32U) {
            ...
            received_count++;
            if (!message.extd && message.identifier == relay_command_can_id(board_cfg.can_start_id)) {
                ESP_LOGW(can_log, "Ignoring external frame using reserved relay command ID 0x%lX",
